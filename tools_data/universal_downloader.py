...
# Set to False to disable the update check entirely (e.g. for testing).
ENABLE_AUTO_UPDATE : bool = True

UPDATE_VERSION_URL : str = "https://raw.githubusercontent.com/BittarMO/Universal-Downloader/master/version.json"
...
class ToolManager:
    ...
    def refresh(self) -> None:
        """Re-resolve both tool paths from disk (call after a download completes)."""
        self.yt_dlp = self._resolve(YT_DLP_BINARY)
        self.ffmpeg = self._resolve(FFMPEG_BINARY)

    @property
    def missing(self) -> List[str]:
        """Return a list of binary names that are not present on disk."""
        result = []
...
class DownloadHistory:
    ...
    def add(self, task: DownloadTask) -> None:
        ...
        with self._lock:
            ...
            if len(self._records) > HISTORY_MAX_ITEMS:
                del self._records[HISTORY_MAX_ITEMS:]
            log.debug("History: added %r  (total=%d)", task.title[:30], len(self._records))
